...
<pre>RECEIPT-PROCESSOR-SYSTEM/
├── main.tf            # Core AWS resources (S3, DynamoDB, Lambda, SES)
├── iam.tf             # IAM roles and policies
├── variables.tf       # Configurable values (email, region, etc.)
├── outputs.tf         # Resource ARNs and IDs for reference
├── terraform.tfvars   # Actual values for your variables
├── provider.tf        # AWS provider configuration
└── src/
    └── lambda_function.py  # Your Python code (from the video)
</pre>
<div align="right"><a href="#readme-top">↑ Back to Top</a></div>
